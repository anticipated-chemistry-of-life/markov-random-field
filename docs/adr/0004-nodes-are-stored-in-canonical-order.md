...
**The promise about previously written files needs re-reading.** The consequence above says internal-state files written before this decision keep working, because rows resolve by node name. Name-keyed resolution does survive ADR-0005 — but the node state now reaches the leaves, so a file written under the old model holds only the internal rows and is *missing* every leaf row rather than merely permuted. Loading one is a migration, not a no-op, and it is scoped as its own piece of work. The column check relaxes with it: a column naming an internal node is no longer required to be one, since the node state legitimately contains leaves.
