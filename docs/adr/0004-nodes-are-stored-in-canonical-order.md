...
**The leaf-index identity is what makes the field-to-node-state correspondence free.** ADR-0005 extends each tree's node state down to its leaves and gives both node-state matrices the field's orientation, so the field and the leaf block of a node state address the same `(row, column)` for a given leaf pair. That correspondence is the identity rather than a conversion *because* of the rule above — a leaf's index in leaf space is its node index — so the `node - n_leaves` arithmetic disappears on the leaf block instead of being replaced by a different offset. (What this record calls the internal state is, from ADR-0005 on, the node state. The node layout fixed here does not move; what grew is the range of it the node state covers.) Reordering the leaf block — or sorting node ids, as the rejected option above proposed — now breaks an indexing identity that two model layers rely on, not merely an output ordering.

**The promise about previously written files needs re-reading.** The consequence above says internal-state files written before this decision keep working, because rows resolve by node name. Name-keyed resolution does survive ADR-0005 — but the node state now reaches the leaves, so a file written under the old model holds only the internal rows and is *missing* every leaf row rather than merely permuted. Loading one is a migration, not a no-op, and it was scoped as its own piece of work (#40). The column check relaxed with it: a column naming an internal node is no longer required to be one, since the node state legitimately contains leaves. A missing leaf row is not an error either. The chain start writes the whole leaf layer after the file is read, so an old file is a warm start for the internals — which is all it ever held. A file written now carries the leaf rows, because the node state spans them.
