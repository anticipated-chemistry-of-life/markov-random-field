...
**The block draws the triple.** The update is a parallel loop over the field's rows — one row per species leaf — and a thread takes a contiguous block of rows, walking each row's columns in ascending molecule-leaf order. Per leaf pair it reads five cells — the three the draw moves and the two tree parents — draws all three from the eight combinations at once, writes back the ones the draw changed, and adds the pair to the six link counters. `block_update::update_cell`, `block_update::update_row` and `block_update::run`, `src/field/TBlockUpdate.h`. The arithmetic stays in `field/TFieldMath.h`.

**The row is the unit because that is what the model can answer once.** A clique of one tree is named by the leaves of every other tree (ADR-0011), so the molecule tree's clique *is* the species leaf: its transition grid, and the species leaf's own branch bin, are constant down a row. The model is asked for them once per row (`begin_row`) and for what is left per cell (`factors`), rather than being asked for everything at every cell. A flat per-cell loop had no place to put that answer; the profile of a 4.6-billion-cell field is what made the difference worth a seam.

**The loop shape decides nothing about the chain.** A leaf pair's uniform is hashed from the field cell's linear index (ADR-0007), and the row's first index plus the molecule leaf *is* that index, so the row loop draws every cell exactly what the flat loop drew it. Verified as byte-identical output over three tree shapes, four storage pairings and four thread counts.

**The leaf pairs are conditionally independent.** A leaf pair's Markov blanket holds no cell of another leaf pair: it holds the two tree parents and the two data terms. A leaf is never a root and an internal node is never a leaf, so no thread writes a cell another leaf pair reads. That is what leaves the split between threads free — rows or cells — and the deferred-insert lists one per thread rather than one per row.
...
**The traversal has since gone back to a row split**, and the reason is the model seam rather than the chain: a row is the granularity at which the molecule tree's transition grid and the species leaf's branch bin can be settled once instead of per cell, which a profile of the real 4.6-billion-cell field showed to be a fifth of the whole run. The deferred-insert list stays one per thread, not one per row. It was checked the same way and by the same standard as the flat loop was: 16 of 19 files byte-identical, same seed, one thread, plus 48 loop-level digests (three tree shapes x four storage pairings x four thread counts) identical to the flat loop's.
