...
_The eight-state block update argued for in the consequences below is the update that runs. [ADR-0008](0008-each-tree-draws-its-own-leaf-layer.md) retired it and drew the leaf layer one variable at a time, and [ADR-0010](0010-the-block-draws-the-leaf-layer.md) reinstated it on the measurement ADR-0008 carries: without the block the field's own factor of the joint density decorrelates 4.7 times slower at `omega = 0.005`, which is where this model's prior concentrates. **So read this record as it was written**, the support argument's lower bound included -- a block update does evaluate `log 0` at `omega = 0`, which ADR-0008 had to re-read and ADR-0010 gives back._
...
**Data indexed on a subset of the dimensions is no longer supported, and reinstating it is a separate problem.** The collapser existed to score a data source that named fewer trees than the field has dimensions. It reduced the field over the unnamed ones by an OR rather than a sum -- the collapsed cell read as one if *any* field cell in it was one, which is the question the per-clique count of leaves in state 1 was maintained to answer, and that counter's only reader. LOTUS records are always indexed on both trees, so this was machinery for a case that never arrived, and it is deleted rather than carried as a code path nothing exercises. Deleting it is what makes the field update local: with no collapsing, a field cell's data likelihood depends on that cell alone, which is the precondition for the eight-state block. Supporting a data source that names only some of the trees is a real future problem, and it needs its own record when someone has data that justifies it -- reinstating it means deciding how a collapsed likelihood interacts with the link, which is a model question rather than a restoration. The `tree_names_to_keep` command-line option went with it, having had nothing left to select.
