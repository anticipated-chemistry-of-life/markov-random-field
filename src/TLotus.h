...
public:
	...
	[[nodiscard]] std::string name() const override { return "lotus_likelihood"; }
	...
	double getSumLogPriorDensity(const Storage &) const override { return _curLL; };
	...
	void fill_tmp_state_along_last_dim(const std::vector<size_t> &start_index_clique_along_last_dim, size_t K) {
		...
		if (_collapser.do_collapse()) {
			_tmp_state_along_last_dim.fill_Y_along_last_dim(_collapser.collapse(start_index_clique_along_last_dim), K,
			                                                _L);
