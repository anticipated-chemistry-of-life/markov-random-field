...
//
// The property that matters is that it redraws each internal node from its full conditional under
// the density tree/node_state_density.h scores: a root from the stationary distribution and its
// children, every other internal node from its parent and its children. The suite therefore takes
// the threshold each node is drawn against from that density, rather than restating the arithmetic
// the walk already carries.
//
// The walk covers internal nodes only. The block update draws a leaf, with the field and the other
// tree's leaf state (ADR-0005). So this suite asserts that no leaf moves, and that no leaf is even
// asked for a uniform.
//
...
/// The probability the density gives node `node` of being a one, with every other node held where
...
double conditional_probability_of_one(const TPhylogeny &topology, const TTransitionGrid &process,
	...
	const double log_zero = log_density_of_clique(topology, process, scored, bin_of);
	scored.set_state(node, true);
	const double log_one = log_density_of_clique(topology, process, scored, bin_of);
...
#pragma omp parallel for num_threads(n_threads) schedule(dynamic) default(none)                    \
	...
	for (size_t k = 0; k < n_cliques; ++k) {
		...
		update_clique(topology, process, bins, uniforms, column);
...
/// Every internal node is visited exactly once, and no leaf is visited at all. A leaf is drawn
/// with the field (ADR-0005), so the walk stops above it.
TEST(NodeStateWalk, visits_every_internal_node_once_and_no_leaf) {
	...
	update_clique(topology, process, TOneBin{1}, uniforms, column);
...
/// A leaf keeps whatever state it was handed. The walk reads it -- a leaf is a child of the node
/// above it -- and never writes it.
TEST(NodeStateWalk, leaves_the_leaf_block_where_it_found_it) {
	const TBinGrid grid(N_BINS);
	const TTransitionGrid process(0.4, 1.0, grid);
	const TPhylogeny topology =
	    build_phylogeny({edge("a", "inner"), edge("b", "inner"), edge("inner", "root")});

	for (const bool leaf_state : {false, true}) {
		TColumn column(topology.n_nodes());
		for (const size_t leaf : topology.leaves()) { column.set_state(leaf, leaf_state); }
		update_clique(topology, process, TOneBin{1},
		              uniforms::TWrittenUniforms(topology.n_nodes(), 0.5), column);

		for (const size_t leaf : topology.leaves()) {
			EXPECT_EQ(column.is_one(leaf), leaf_state)
			    << "leaf " << topology.id_of(leaf) << " moved";
		}
	}
}

// -------------------------------------------------------------------------
// It draws the conditional the density states
// -------------------------------------------------------------------------
...
TEST(NodeStateWalk, draws_a_root_from_the_stationary_distribution_and_its_children) {
	...
	// One root over two leaves, so the root is the only node the walk touches.
	...
	for (const size_t leaves_mask : {0b00U, 0b01U, 0b11U}) {
		...
		const double threshold =
		    conditional_probability_of_one(topology, process, bins, held, root);
...
TEST(NodeStateWalk, draws_a_non_root_from_its_parent_and_its_children) {
	...
	for (const bool root_state : {false, true}) {
		for (const size_t leaves_mask : {0b00U, 0b01U, 0b11U}) {
			...
			const double threshold =
			    conditional_probability_of_one(topology, process, bins, held, inner);
			ASSERT_GT(threshold, 0.0);
			ASSERT_LT(threshold, 1.0);
			...
			for (const auto &[uniform, expected] :
				...
				uniforms::TWrittenUniforms uniforms(topology.n_nodes(), 0.5);
				// The root's own draw is held where it started, so the inner node reads the state
				// this update began with.
...
/// Every internal node, whatever its kind and whatever the configuration around it, is drawn
/// against the threshold the density names. One assertion over the whole walk, so a node the walk
/// scores against the wrong neighbours has nowhere to hide.
TEST(NodeStateWalk, draws_every_internal_node_against_the_density_threshold) {
	const TBinGrid grid(N_BINS);
	...
	for (size_t replicate = 0; replicate < 40; ++replicate) {
		...
		update_clique(topology, process, bins, uniforms, walked);

		// The same walk, node by node, with each threshold taken from the density over the states
		...
		for (const size_t node : topology.internal_nodes()) {
			const double threshold =
			    conditional_probability_of_one(topology, process, bins, scored, node);
...
TEST(NodeStateWalk, a_cell_draws_the_uniform_its_own_linear_index_names) {
	const TBinGrid grid(N_BINS);
	const TTransitionGrid process(0.4, 1.0, grid);
	// Two roots, so a root can be moved without any other node reading it.
	...
	update_clique(topology, process, TOneBin{1}, uniforms, before);
	...
	update_clique(topology, process, TOneBin{1}, uniforms, after);
...
/// with the number of threads that walk them. Each clique reads its own cells and its own
/// uniforms, so the answer is a property of the cell and not of the schedule (ADR-0007).
