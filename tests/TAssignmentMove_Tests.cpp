...
TEST(TAssignmentMove_Tests, initialize_to_unknown) {
	auto run = make_run();
...
TEST(TAssignmentMove_Tests, initialize_requires_matching_unknown_sizes) {
	TMassSpecRun run;
...
TEST(TAssignmentMove_Tests, apply_revert_to_unknown) {
	auto run = make_run();
...
TEST(TAssignmentMove_Tests, swap_apply_revert) {
	auto run = make_run();
	run.set_current_assignment(0, TFeatureLikelihood(10, 100));
...
TEST(TAssignmentMove_Tests, swap_with_unknown_apply_revert) {
	auto run = make_run();
...
TEST(TAssignmentMove_Tests, touches_two_features_covers_both_swap_types) {
	TAssignmentProposal p;
	p.type = AssignmentMoveType::Swap;
...
TEST(TAssignmentMove_Tests, add_likelihood_vector_rejects_zero_binned_likelihood) {
	TMassSpecRun run;
...
TEST(TAssignmentMove_Tests, add_likelihood_vector_rejects_duplicate_molecules) {
	TMassSpecRun run;
	std::vector<TFeatureLikelihood> f0 = {TFeatureLikelihood(10, 100), TFeatureLikelihood(10, 90)};
	EXPECT_ANY_THROW(run.add_likelihood_vector(f0));
}

TEST(TAssignmentMove_Tests, initialize_to_unknown_rejects_zero_probability) {
	TMassSpecRun run;
	std::vector<TFeatureLikelihood> f0 = {TFeatureLikelihood(10, 100)};
	run.add_likelihood_vector(f0);
