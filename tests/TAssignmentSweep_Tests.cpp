...
/// `+ACCEPT_ALL` makes the sweep accept everything, `-ACCEPT_ALL` makes it reject everything while
...
TEST(TAssignmentSweep_Tests, sweep_reject_all_leaves_state_unchanged) {
...
TEST(TAssignmentSweep_Tests, sweep_accept_all_preserves_invariant) {
	coretools::instances::randomGenerator().setSeed(42, true);
	auto run = make_overlapping_run();
	...
	for (int sweep = 0; sweep < 200; ++sweep) {
		run.update_all_assignments(scorer, 0.1);
		ASSERT_TRUE(no_molecule_assigned_twice(run)) << "after sweep " << sweep;
		ASSERT_TRUE(every_assignment_is_an_own_candidate(run)) << "after sweep " << sweep;
	}
	// with everything accepted the run must actually have moved away from the all-unknown start
	EXPECT_FALSE(scorer.seen.empty());
}

// The features are drawn at random, so a sweep makes as many attempts as there are features but
// visits them in no particular order and may repeat one. Over many sweeps every feature must come
// up.
TEST(TAssignmentSweep_Tests, sweep_makes_one_attempt_per_feature) {
	coretools::instances::randomGenerator().setSeed(42, true);
	...
	for (int sweep = 0; sweep < 50; ++sweep) { run.update_all_assignments(scorer, 0.1); }
...
TEST(TAssignmentSweep_Tests, hastings_from_unknown) {
	coretools::instances::randomGenerator().setSeed(42, true);
...
TEST(TAssignmentSweep_Tests, hastings_to_unknown) {
	coretools::instances::randomGenerator().setSeed(42, true);
	auto run = make_overlapping_run();
...
TEST(TAssignmentSweep_Tests, hastings_zero_for_move_to_free) {
	coretools::instances::randomGenerator().setSeed(42, true);
	...
	for (int sweep = 0; sweep < 50; ++sweep) { run.update_all_assignments(scorer, 0.0); }
...
TEST(TAssignmentSweep_Tests, hastings_zero_for_swap) {
	coretools::instances::randomGenerator().setSeed(42, true);
...
TEST(TAssignmentSweep_Tests, hastings_swap_with_unknown) {
	coretools::instances::randomGenerator().setSeed(42, true);
...
TEST(TAssignmentSweep_Tests, illegal_swap_is_skipped) {
	coretools::instances::randomGenerator().setSeed(42, true);
...
// The holder map is rebuilt at the start of every sweep and throws if it finds a molecule held by
// two features, so a bookkeeping bug in one sweep surfaces at the start of the next one.
TEST(TAssignmentSweep_Tests, holder_map_stays_in_sync_across_sweeps) {
	coretools::instances::randomGenerator().setSeed(42, true);
	auto run = make_overlapping_run();
	...
	for (int sweep = 0; sweep < 50; ++sweep) {
		EXPECT_NO_THROW(run.update_all_assignments(scorer, 0.1)) << "at sweep " << sweep;
	}
}

TEST(TAssignmentSweep_Tests, sweep_on_run_with_zero_features_is_noop) {
	TMassSpecRun run;
...
TEST(TAssignmentSweep_Tests, sweep_throws_when_number_of_molecules_unset) {
	TMassSpecRun run;
...
TEST(TAssignmentSweep_Tests, sweep_throws_when_assignments_were_not_initialized) {
	TMassSpecRun run;
	std::vector<TFeatureLikelihood> f0 = {TFeatureLikelihood(10, 100)};
	run.add_likelihood_vector(f0);
	run.set_number_of_molecules(NUMBER_OF_MOLECULES);
...
TEST(TAssignmentSweep_Tests, sweep_does_not_throw_when_all_features_unknown) {
	coretools::instances::randomGenerator().setSeed(42, true);
	auto run = make_overlapping_run();
	for (size_t f = 0; f < run.number_of_assignments(); ++f) {
...
TEST(TAssignmentSweep_Tests, feature_with_no_candidates_is_skipped) {
	coretools::instances::randomGenerator().setSeed(42, true);
	...
	TFixedScorer scorer;
	scorer.value = ACCEPT_ALL;
	for (int sweep = 0; sweep < 50; ++sweep) { run.update_all_assignments(scorer, 0.5); }

	EXPECT_TRUE(scorer.seen.empty());
	EXPECT_TRUE(same_assignments(before, snapshot(run)));
}

TEST(TAssignmentSweep_Tests, beta_zero_never_proposes_to_unknown) {
	coretools::instances::randomGenerator().setSeed(42, true);
	auto run = make_overlapping_run();
	run.set_current_assignment(0, TFeatureLikelihood(10, 100));
	run.set_current_assignment(1, TFeatureLikelihood(20, 80));
...
TEST(TAssignmentSweep_Tests, beta_one_always_proposes_to_unknown) {
	coretools::instances::randomGenerator().setSeed(42, true);
	auto run = make_overlapping_run();
	run.set_current_assignment(0, TFeatureLikelihood(10, 100));
	run.set_current_assignment(1, TFeatureLikelihood(20, 80));
...
TEST(TAssignmentSweep_Tests, identity_pick_is_skipped) {
	coretools::instances::randomGenerator().setSeed(42, true);
