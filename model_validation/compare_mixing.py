...
def _chains(workdir: pathlib.Path, binary: str) -> list[tuple[str, pathlib.Path]]:
    ...
    directories = sorted(
        (workdir / binary).glob("seed_*"), key=lambda p: int(p.name.split("_")[1])
    )
    if not directories:
        raise click.ClickException(
            f"{workdir / binary} holds no seed_* chain directory."
        )
...
def _report_traces(chains, meta, report) -> dict[str, dict[str, list[np.ndarray]]]:
    ...
    click.echo(
        f"{'binary':<10} {'seed':>5} {'samples':>8} {'mean':>14} {'acf(1)':>8} "
    ...
    for binary in BINARIES:
        ...
        for seed, directory in chains[binary]:
            ...
            click.echo(
                f"{binary:<10} {seed:>5} {summary.n_samples:>8} {summary.mean:>14.1f} "
...
def _report_factors(factors, report) -> None:
    ...
    click.echo(
        f"{'factor':<22} {'binary':<10} {'acf(1)':>8} {'median tau':>11} {'ESS':>8} "
        f"{'drift':>7} {'ratio':>7}"
    )
    report["factors"] = {}
    for name in factors[REFERENCE]:
        ...
        for binary in BINARIES:
            ...
            medians[binary] = statistics.median(
                [s.autocorrelation_time for s in per_chain]
            )
    ...
    for name in (TOTAL, FIELD_FACTOR):
        ...
        for binary in BINARIES:
            curves = [
                autocorrelation(v, max_lag=max(ACF_LAGS)) for v in factors[binary][name]
            ]
...
def _report_totals(chains, summaries, report) -> None:
    ...
    ess_per_second = {
        b: [
            report["traces"][b][seed]["effective_samples_per_second"]
            for seed, _ in chains[b]
        ]
        for b in BINARIES
    }
    seconds = {
        b: [report["traces"][b][seed]["seconds"] for seed, _ in chains[b]]
        for b in BINARIES
    }
    click.echo(
    ...
    click.echo(
        f"  seconds per chain  reference {statistics.median(seconds[REFERENCE]):9.1f}   "
        f"current {statistics.median(seconds[CURRENT]):9.1f}"
    )
    report["summary"] = {
        "median_autocorrelation_time": {
            b: statistics.median(taus[b]) for b in BINARIES
        },
...
def _report_posteriors(chains, meta, trees, report) -> dict[str, dict[str, list]]:
    ...
    for tree in trees:
        for binary in BINARIES:
            ...
            click.echo(
                f"{tree:<12} {binary:<10} {statistics.mean(densities):>9.5f} "
    ...
    for tree in trees:
        ...
        click.echo(
            f"{tree:<12} {agreement.n_cells:>7} {agreement.mean_abs_diff:>12.5f} "
...
def _report_ridge(chains, posteriors, trees, report) -> None:
    ...
    click.echo("\n== the ADR-0005 ridge ==")
    click.echo(
        f"{'binary':<10} {'seed':>5} {'a~_s':>9} {'a~_m':>9} {'product':>10} "
        f"{'along':>9} {'across':>9}"
    )
    ...
    for binary in BINARIES:
        ...
        for index, (seed, _) in enumerate(chains[binary]):
            point = ridge_point(
                posteriors[binary][trees[0]][index], posteriors[binary][trees[1]][index]
            )
            points[binary].append(point)
            click.echo(
                f"{binary:<10} {seed:>5} {point.species:>9.5f} "
        ...
        report["ridge"][binary] = [
            {
                ...
                "product": p.product,
                "log_ratio": p.log_ratio,
                "log_product": p.log_product,
            }
    ...
    for binary in BINARIES:
        ...
        report["ridge"].setdefault("spread", {})[binary] = {
            "along": along,
            "across": across,
        }
    ...
    click.echo(
        f"    across it        {shift.across:+.4f}   (the field's own density moving)"
    )
...
@click.argument(
    "workdir", type=click.Path(exists=True, file_okay=False, path_type=pathlib.Path)
)
@click.option(
    "--json",
    "json_path",
...
def main(workdir: pathlib.Path, json_path: pathlib.Path | None) -> None:
    ...
    click.echo(
        f"The data set is fixed and the seed is not: {meta['n_replicates']} chains per "
        f"binary, {meta['iterations']} iterations each, on one data set simulated by the "
        f"reference binary at seed {meta['simulate_seed']}."
    )
    click.echo(
        f"  reference {meta['reference_revision']}   current {meta['current_revision']}"
    )
    click.echo(
        f"  {meta['n_species_leaves']} x {meta['n_molecules_leaves']} = "
